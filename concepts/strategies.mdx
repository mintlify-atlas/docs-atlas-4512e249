---
title: "Rate-Limiting Strategies in ThrottleKit"
...
A *shaping* variant that **delays** rather than rejects, smoothing output to a fixed drain rate. Useful for outbound rate shaping (e.g., a third-party API budget). `limiter.schedule(key)` resolves when a slot is available, with a `maxQueueMs` ceiling that rejects rather than queuing indefinitely.
