...
```ts
import { buildRateLimitHeaders } from "throttlekit/headers";
...
```ts
import { createEnforcer } from "throttlekit/enforce";
...
const key = hashKey(rawIp); // SHA-256 hex, truncated to 32 chars
```
